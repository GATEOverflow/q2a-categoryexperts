...
select categoryid from ^categories where parentid in (select categoryid from ^categories where parentid = b.categoryid)))";
	$query = "insert into ^catpoints (categoryid, userid, points, netvotes, aselects)  (select  b.categoryid,$, ".
...
(select  ".$calculations['aselecteds']['formula'].$catfilter.") as aselects
 from  ^categories b) on duplicate key update ^catpoints.points=points, ^catpoints.netvotes=netvotes, ^catpoints.aselects=aselects";
